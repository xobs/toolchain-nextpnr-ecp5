...
rsync -a $nextpnr_dir $BUILD_DIR/$nextpnr_dir --exclude .git
rsync -a $prjtrellis_dir $BUILD_DIR/$prjtrellis_dir --exclude .git
...
if [ $ARCH == "darwin" ]; then
    ...
    cmake -DBUILD_SHARED=OFF -DSTATIC_BUILD=OFF .
